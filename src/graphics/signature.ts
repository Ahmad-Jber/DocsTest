...
export const signature: Plugin<Signature> = {
  ui: async (arg) => {
    ...
    try {
      value ? await signaturePad.fromDataURL(value, {ratio: resetScale}) : signaturePad.clear();
    } catch (e) {
      console.error(e);
    }
    ...
    if (mode === 'viewer') {
      ...
      signaturePad.on();
      if (value==signature.propPanel.defaultValue){
        signaturePad.clear()
      }
      ...
      clearButton.style.backgroundColor = "#008580"
      clearButton.style.color = "#fff"
      clearButton.textContent = i18n('clear') || 'clear';
  ...
  propPanel: {
    ...
    defaultSchema: { type: 'signature', position: { x: 100, y: 50 }, width: 100, height: 100 },
  },
};
